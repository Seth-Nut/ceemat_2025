...
# Interfaz de usuario
ui <- fluidPage(
  titlePanel("📊 Gráfico Interactivo con ggplot2"),
  sliderInput("line_width", "Grosor de línea:", min = 1, max = 5, value = 2, step = 0.5),
  plotOutput("plot")
)
...
server <- function(input, output) {
  output$plot <- renderPlot({
    ggplot(df, aes(x = x, y = y)) +
      geom_line(size = input$line_width, color = "blue") +
      geom_point(size = 3, color = "red") +
      labs(title = "Gráfico de Ejemplo", x = "Eje X", y = "Eje Y") +
      theme_minimal()
  })
}
